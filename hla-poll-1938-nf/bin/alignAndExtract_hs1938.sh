...
#
# modified by Xinming Zhuo for compatible with hg19
...
hg19_chk=$($samtools_bin view -H $bam_path | grep ^@SQ | grep -E "hs37|hg19" | wc -l)
if [ $hg19_chk -gt 0 ]; then 
    
    ref_fasta=$3
    echo ">>>>>>>>>>>>>>>> extracting reads mapping to HLA loci and ALT contigs (hg19)"
    samtools view -b $bam_path \
        6:29691117-29695073 6:29694378-29716826 6:29794756-29798899 6:29855537-29858857 \
        6:29910247-29913661 6:29973748-29977733 6:30227339-30234728 6:30457183-30461982 \
        6:31236526-31239913 6:31321643-31325022 6:31367561-31383093 6:31462658-31478901 \
        6:32407619-32412828 6:32485154-32498006 6:32520490-32527779 6:32546547-32557613 \
        6:32605183-32611429 6:32627241-32634466 6:32709163-32714664 6:32723875-32731330 \
        6:32780540-32784825 6:32789610-32806547 6:32812986-32820564 6:32902406-32908847 \
        6:32916391-32920899 6:32971960-32977389 6:33032346-33048555 6:33043703-33057473 \
        6:33080293-33096890 \
        | $samtools_bin view -b -F 0x4 - | $samtools_bin sort --thread $num_processors -m $samtools_sort_memory_per_thread -O BAM - > $sampleid.tmp.extract.bam

    #rm $sample.bam $sample.*bai 
    #echo ">>>>>>>>>>>>>> indexing extracted bam (hg19)"
    #$samtools_bin index $sampleid.tmp.extract.bam
    #convert to fastq and remapped to hg38   
    ...
    #samtools collate -Oun128 $sammple.extract.bam | samtools fastq -OT RG,BC - \
    #| bwa mem -K 100000000 -p -v 3 -t 4 -CH <(samtools view -H $sammple.extract.bam |grep ^@RG) -M -Y ${ref_fasta} - \
    #| samtools sort -@4 -m4g -o $sample.bam -
else
