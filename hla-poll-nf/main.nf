...
params.index_description = "index bam file"
params.index_script = ""
params.index_dockerimg = "xmzhuo/hla:0.0.9"
params.index_argument = "sample=\$(echo !{bam} | sed 's/.bam/.hla.bam/'); cp !{bam} \${sample}; /samtools/bin/samtools index \${sample}"
params.index_outputDir = "./results/hla-poll"
params.index_sakcpu = "4"
params.index_sakmem = "4.GB"
params.index_saktime = "1.hour"
params.index_input_bam = "/path/to/*.bam"
params.hla_name = "hla"
params.hla_description = "multiple hla caller"
params.hla_script = ""
params.hla_dockerimg = "xmzhuo/hla:0.0.9"
params.hla_argument = "bash hla_poll_v1.8.sub.sh !{index_bam} \$(pwd)"
params.hla_outputDir = "./results/hla-poll"
params.hla_sakcpu = "8"
params.hla_sakmem = "16.GB"
params.hla_saktime = "2.hour"
params.hla_input_file = ""
...
params.polysolver_argument = "bash hla_poll_polysolver.sh !{index_bam}"
params.polysolver_outputDir = "./results/hla-poll"
params.polysolver_sakcpu = "8"
params.polysolver_sakmem = "16.GB"
...
params.poll_script = ""
params.poll_dockerimg = "xmzhuo/hla:0.0.9"
...

{
  ...
  'argument': 'sample=\$(echo !{bam} | sed 's/.bam/.hla.bam/'); cp !{bam} \${sample}; /samtools/bin/samtools index \${sample}',
...
}
{
  ...
  'script': '',
  'dockerimg': 'xmzhuo/hla:0.0.9',
  'argument': 'bash hla_poll_v1.8.sub.sh !{index_bam} \$(pwd)',
  'outputDir': './results/hla-poll',
  'sakcpu': '8',
  'sakmem': '16.GB',
  'saktime': '2.hour'
}
{
  ...
  'argument': 'bash hla_poll_polysolver.sh !{index_bam}',
  'outputDir': './results/hla-poll',
  'sakcpu': '8',
  'sakmem': '16.GB',
  'saktime': '2.hour'
...
include { INDEXDOC } from './modules/index_docker'
...
workflow {
    ...
    */
    ...
    if(params.index_input_bam) {INDEX_bam = Channel.fromPath(params.index_input_bam).toSortedList()} else {INDEX_bam = Channel.fromPath("$baseDir" + "/data/input_sanitychk")}
    if(params.index_script) {INDEX_ScriptFiles = Channel.fromPath(params.index_script).toSortedList()} else {INDEX_ScriptFiles = Channel.fromPath("$baseDir" + "/bin/script_sanitychk")}
    
    if (params.index_dockerimg) {
        INDEXDOC(INDEX_bam,  INDEX_ScriptFiles, params.index_argument, params.index_dockerimg, params.index_sakcpu, params.index_sakmem, params.index_saktime, params.index_outputDir)
    } else {
        INDEX(INDEX_bam,  INDEX_ScriptFiles, params.index_argument, params.index_sakcpu, params.index_sakmem, params.index_saktime, params.index_outputDir)   
    }
    //* ## step cmd example SAK
    if(params.hla_input_file) {HLA_file = Channel.fromPath(params.hla_input_file).toSortedList()} else {HLA_file = Channel.fromPath("$baseDir" + "/data/input_sanitychk")}
    ...
    if (params.hla_dockerimg) {
        HLADOC(HLA_file, INDEXDOC.out.bam.collect(), INDEXDOC.out.bai.collect(), HLA_ScriptFiles, params.hla_argument, params.hla_dockerimg, params.hla_sakcpu, params.hla_sakmem, params.hla_saktime, params.hla_outputDir)
    } else {
        HLA(HLA_file, INDEXDOC.out.bam.collect(), INDEXDOC.out.bai.collect(), HLA_ScriptFiles, params.hla_argument, params.hla_sakcpu, params.hla_sakmem, params.hla_saktime, params.hla_outputDir)   
    }
    //* ## step cmd example SAK
    ...
    if (params.polysolver_dockerimg) {
        POLYSOLVERDOC(POLYSOLVER_file, INDEXDOC.out.bam.collect(), INDEXDOC.out.bai.collect(), POLYSOLVER_ScriptFiles, params.polysolver_argument, params.polysolver_dockerimg, params.polysolver_sakcpu, params.polysolver_sakmem, params.polysolver_saktime, params.polysolver_outputDir)
    } else {
        POLYSOLVER(POLYSOLVER_file, INDEXDOC.out.bam.collect(), INDEXDOC.out.bai.collect(), POLYSOLVER_ScriptFiles, params.polysolver_argument, params.polysolver_sakcpu, params.polysolver_sakmem, params.polysolver_saktime, params.polysolver_outputDir)   
    }
    //* ## step cmd example SAK
