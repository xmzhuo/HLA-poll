...
#### hla_poll_v1.8.1.seq_bg.run.sh
###############################
#extra chk point for polysolver fail (such as single read), avoid stall
######## body of code ########
  ...
  #for sample in $(ls *.bam); do
    ...
    if [ 1 == 1 ]; then
        ...
        while :; do
            #check container status, if fail then abondon the polysolver call 
            con_sta_chk=$(docker container ps | grep -w $container_NAME | wc -l)
            if [ $con_sta_chk -eq 0 ]; then break; else sleep 10; fi
